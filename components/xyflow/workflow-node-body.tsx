...
export const WorkflowNodeBody = ({ nodeKind }: WorkflowNodeBodyProps) => {
    ...
    case "url-input":
      return (
        <div className="space-y-2">
          <textarea
            value={pipeline.urlText}
            onChange={(event) => pipeline.setUrlText(event.target.value)}
            rows={4}
          ...
          <p className="text-[10px] text-zinc-500">
            {pipeline.allUrls.length} URLs · quick probe uses 1 at a time (up to 5)
          </p>
        </div>
      )
        ...
        <div className="space-y-2 text-[10px] text-zinc-400">
          <label className="flex items-center justify-between gap-2">
            ...
            <input
              ...
              max={1}
              step={0.05}
              value={pipeline.confidenceThreshold}
              onChange={(event) =>
                pipeline.setConfidenceThreshold(Number(event.target.value))
              }
          ...
          {pipeline.discovery ? (
            ...
            <p>Play to inspect 1 URL, then add more only if confidence is low.</p>
          )}
        </div>
      )
